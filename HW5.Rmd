...
nums = sample(1:nrow(abalone), floor(nrow(abalone)*.7))
train = abalone[nums,]
test = abalone[-nums,]
...
oob.err = double(ncol(train))
test.err = double(ncol(train))

#In a loop of mtry from 1 to 13, you first fit the randomForest to the train dataset
for(mtry in 1:13){
  fit = randomForest(sex ~ ., data = train, mtry=mtry, ntree = 350)
  oob.err[mtry] = fit$err.rate[350,1] ##extract Mean-squared-error 
  pred = predict(fit, test) #predict on test dataset
  test.err[mtry] = with(test, mean( (medv-pred)^2 )) #compute test error
}
